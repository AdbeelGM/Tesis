...
function bindSectionChestClicks(root = document) {
  ...
  chests.forEach((chest) => {
    ...
    chest.addEventListener('click', () => {
      if (chest.disabled) {
        console.warn('Completa todos los niveles de esta sección para abrir el cofre.');
        return;
      }
  ...
  buyButtons.forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      if ((productId === 'single_heart' || productId === 'heart_bundle') && livesAtMax) {
        console.warn('Ya tienes el máximo de corazones.');
        updateStorePurchaseAvailability(state);
        return;
      }
      if (productId === 'infinite_hearts_24h' && state.infiniteHeartsActive) {
        console.warn('Ya tienes corazones ilimitados activos.');
        updateStorePurchaseAvailability(state);
        return;
      }
